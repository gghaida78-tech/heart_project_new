...
# @st.cache_resource(show_spinner=False)ِ
#     possible_paths = [
...
# model = load_model()
#
# # إعداد الصفحة (مرة واحدة في الكود)
# st.set_page_config(
...
#     """
#     <style>
#     .stApp { background-color: white; color: black; direction: rtl; text-align: right; }
#     h1, h2, h3, h4, h5, h6, p, label, .stMarkdown p { font-family: 'Arial', 'Segoe UI', Tahoma, sans-serif; font-size: 20px; color: #000000; text-align: right; }
#     input, textarea, select { direction: rtl !important; text-align: right !important; }
#     .stTextInput > div, .stButton > button { max-width: 350px; }
#     .stSlider > div, .stSelectbox > div, .stRadio > div { max-width: 500px; }
#     </style>
#     """,
...
#         nav = st.radio("التنقّل", ["الصفحة الرئيسية", "نموذج التنبؤ", "حول"], index=["الصفحة الرئيسية", "نموذج التنبؤ", "حول"].index(st.session_state.nav) if st.session_state.nav in ["الصفحة الرئيسية", "نموذج التنبؤ",  "حول"] else 0)
...
#                 st.session_state.nav = "الصفحة الرئيسية"
#                 st.toast(f"مرحباً {st.session_state.username}! تم تسجيل الدخول بنجاح 👋", icon="✅")
#                 st.experimental_rerun()
#             else:
...
#     st.header("💓 نظام تنبؤ أمراض القلب")
#     col1, col2 = st.columns([1, 2])
#     with col1:
#         st.subheader("لماذا هذا التطبيق؟")
#         st.markdown(
#             """
#             - يساعدك على تقدير احتمال وجود خطر مرض قلبي بناءً على 13 سمة إكلينيكية.
#             - يقدم نتيجة فورية مع نسبة خطر تقريبية وتمثيل بصري.
#             """
#         )
#         st.info("نذكّرك أن النتائج إرشادية وليست بديلاً عن التشخيص الطبي.")
#     with col2:
#         st.markdown("### كيفية الاستخدام")
#         st.markdown(
#             """
#             1. من القائمة الجانبية اختر "نموذج التنبؤ"، ثم أدخل بياناتك.
#             2. اضغط "تنبؤ" لعرض النتيجة ونسبة الخطر.
#             """
#         )
#
# # نموذج التنبؤ (فردي)
...
#         st.warning(
#             "النموذج الحالي تم تدريبه على ميزات تختلف عن واجهة الإدخال."
#             " يفضل إعادة تدريب النموذج بهذه الميزات: " + ", ".join(required_features)
#         )
#
#     c1, c2, c3 = st.columns(3)
#     with c1:
#         age = st.number_input("👶 العمر", min_value=18, max_value=120, value=30, help="العمر بالسنوات")
#         chol = st.number_input("🥓 الكوليسترول (mg/dl)", min_value=80, max_value=700, value=200)
#         restecg = st.selectbox("📈 نتائج تخطيط القلب", [0, 1, 2], help="قيم 0-2 حسب التصنيف الطبي")
#         oldpeak = st.number_input("📉 انخفاض ST", min_value=0.0, max_value=10.0, value=1.0, step=0.1)
#     with c2:
#         sex = st.radio("🚻 الجنس", ["ذكر", "أنثى"], help="ذكر=1، أنثى=0")
#         fbs = st.radio("🍬 سكر صائم > 120 mg/dl", [0, 1])
#         thalachh = st.number_input("❤️ أقصى معدل ضربات القلب", min_value=60, max_value=220, value=150)
#         slp = st.selectbox("📊 ميل مقطع ST", [0, 1, 2])
#     with c3:
#         cp = st.selectbox("💓 نوع ألم الصدر", [0, 1, 2, 3])
#         trtbps = st.number_input("🩸 ضغط الدم الانقباضي (mm Hg)", min_value=80, max_value=250, value=120)
...
#     st.markdown(
#         """
#         تطبيق تجريبي للتنبؤ بخطر أمراض القلب بالاعتماد على نموذج تعلم آلة.
#         - الهدف: التوعية وإعطاء مؤشر أولي فقط.
#         - التقنية: Streamlit + scikit-learn + Plotly.
#         - ملاحظة: لا يُعد هذا التشخيص بديلاً عن استشارة الطبيب.
#         """
#     )'''
#
#
